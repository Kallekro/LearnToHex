...
public:
	...
	void step(ObjectiveFunctionType const& function){
		...
		fvar /= offspring.size() - 2;
		
		double cztest = m_rate * 0.02;
		
		//update noise statistics
		m_ztest = (1-cztest)*m_ztest + cztest * fvar/var_noise;
		m_rate = 1.5/(1.5+1.0/(m_ztest-1.0));
...
protected:
	...
	void updatePopulation( std::vector<IndividualType > const& offspring){	
		...
		double cV = m_rate * muEff / m_numberOfVariables;
		double cPath = 2*(muEff * m_rate+ 2.)/(m_numberOfVariables + muEff * m_rate+ 5.);
